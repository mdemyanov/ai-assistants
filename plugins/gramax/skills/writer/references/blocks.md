...
## Виды (Views) — списки дочерних

```markdown
<view defs="hierarchy=none" display="List"/>
```

Генерирует список дочерних страниц. Размещать на отдельной строке в `_index.md` раздела.

Параметры:
- `defs="hierarchy=none"` — плоский список
- `display="List"` — вертикальный список
